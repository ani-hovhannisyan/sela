...
/**
 * "task" stands for one new item in the news list of Notion database.
...
setInitialTaskPageIdToStatusMap().then(() => {
  setInterval(prepareTaskForHugo, 10000)
})
...
async function setInitialTaskPageIdToStatusMap() {
  console.log("1.>> Prepare initial task page id")
  const currentTasks = await getTasksFromNotionDatabase()
  for (const { pageId, status } of currentTasks) {
    console.log('IN setInitialTaskPageIdToStatusMap,  page Id:', pageId)
...
async function prepareTaskForHugo() {
  console.log(">>NUM1")
  ...
  for (const task of updatedTasks) {
    ...
    pid = task.pageId
    await pushTasktoHugoGithub(task, pid)
  }
}
...
async function getTasksFromNotionDatabase() {
  console.log("2.>>Getting tasks from database")
  const pages = []
  ...
  for (const page of pages) {
    ...
    //console.log('\n\n\nCURRENT PAGES ARE:', page.properties.TitleJapanese.title[0])
    console.log('\n\n\nCURRENT PAGES ARE:', page.id)
    getPage(page.id)

    //const statusPropertyId = page.properties["Status"].id
    const statusPropertyId = page.properties["TitleJapanese"].id
    ...
    const titlePropertyId = page.properties["TitleJapanese"].id
    ...
    //console.log('\n\n\n ALL tasks as news items are:', tasks)
  }
 ...
 */
async function pushTasktoHugoGithub({ title, status }, pid) {
console.log(">>NUM4")
...
async function getPage(pageId) {
  ...
  //pageId = 'fb8a5cda79d74a51af71373b45209647'

  console.log('\n\n\n\n RETRIEVEING PAGE:', pageId);
  const response = await notion.pages.retrieve({ page_id: pageId });
